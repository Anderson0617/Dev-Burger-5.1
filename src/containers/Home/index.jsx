...
export default function Home() {
	...
	const { isOpen: isStoreOpen, countdown } = useCountdown();
	...
	}, [isDrawerOpen, isPixModalOpen, isQuantityModalOpen, isAddonModalOpen, isFlavorModalOpen]);
		...
		setCart((prev) => {
			...
			if (existing) {
				return prev.map((item) =>
					item.id === finalProduct.id ? { ...item, quantity: item.quantity + quantity } : item
				);
			}
	...
	const handleClosePixModal = () => {
		...
		setCart([]);
		setFormData({ name: "", phone: "", address: "", observation: "", change: "" });
	...
	const handleCashCheckout = () => {
		if (cart.length > 0) {
			...
			setDeliveryType("pickup");
			setDrawerOpen(false);
			showToast("Pedido enviado! (Pagamento em dinheiro)", 3000, "success");
		}
	};
	...
	return (
		<AppContainer>
			<ToastManager>
				{toasts.map((toast) => (
					<Toast key={toast.id} id={toast.id} message={toast.message} duration={toast.duration} type={toast.type} onClose={handleCloseToast} />
				))}
			</ToastManager>

			<Header>
				<HeaderTitle>A vitrine para seu negócio brilhar</HeaderTitle>
				<HeaderSlogan>Aberto de Segunda à Sábado, 10:00 - 16:00 e 18:00 - 00:00</HeaderSlogan>
				<StatusBadge
					$isOpen={isStoreOpen}
					style={{
						width: "243px",
				...
				>
					{isStoreOpen ? "Aberto" : "Fechado"} {/* anderson aqui */}
				</StatusBadge>
			</Header>

			<MainContent>
				<MenuSection>
					<DishOfTheDayTitle>Dev-Burger-5.1</DishOfTheDayTitle>
					<DishOfTheDayCard
						...
						<ProductList>
							{products.map((product) => (
								<ProductCard
									...
									isStoreOpen={isStoreOpen}
									countdown={countdown} // anderson aqui
								/>
							))}
						</ProductList>
					</MenuSection>
			...
			<FloatingCartButton itemCount={cartItemCount} onClick={handleFloatingCartClick} />

			<QuantityModal isOpen={isQuantityModalOpen} onClose={() => setIsQuantityModalOpen(false)} product={selectedProductForQuantity} onConfirm={handleAddToCart} />
			<AddonModal isOpen={isAddonModalOpen} onClose={() => setIsAddonModalOpen(false)} product={selectedProductForAddon} onConfirm={handleConfirmAddon} />
			<FlavorModal isOpen={isFlavorModalOpen} onClose={() => setIsFlavorModalOpen(false)} onConfirm={handleConfirmFlavor} />
